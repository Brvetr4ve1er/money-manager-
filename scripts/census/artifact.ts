...
export interface Census {
  ...
  tool: string
  capturedAt: string
  tree: { sha: string; branch: string; dirty: boolean }
  inputsHash: string
  env: {
    node: string
    chrome: string
...
export function censusPixels(rgb: Uint8Array, palette: Token[]): PixelCensus {
  ...
  for (let key = 0; key < histogram.length; key++) {
    ...
    if (match.deltaE > STRAY_DELTA_E) strayPixels += count
    if (match.deltaE > 0) {
      offPalette.push({
  ...
  offPalette.sort((a, b) => (b.pct - a.pct) || (a.hex < b.hex ? -1 : 1))
  ...
  return {
    ...
    strays: {
      ...
      top: offPalette.slice(0, 5).map((s) => ({
        hex: s.hex,
        pct: round2(s.pct),
        nearest: s.nearest,
...
export function censusScrollingForm(
  ...
  return {
    ...
    breaches: scrollingFormBreaches(windows, mean, paper === 0 ? 0 : (inkPixels / paper) * 100, documentBuckets),
    windows,
  }
}
 ...
 * numbers in this same file say why. See the constant below — it ships inside
 * the artifact so the argument travels with the numbers.
 */
export const BUCKET_NOTE =
  'Sand and Fog sit in BONE, and Void sits in GRAPHITE. Checked against round 4 ' +
  "(the tree of commit a2d4e6d, re-shot 2026-08-04): this tool's app.375x812.light.seeded " +
  'reads 51.2/40.9/4.8/3.1 against round 4\'s 52.9/40.6/4.1/2.4 — inside 2pp on every ' +
  'bucket, so the mapping reproduces the historical method where the two can be compared. ' +
  'The dark phone does NOT reproduce (this tool 51.4/32.8/12.5/3.2 against round 4\'s ' +
  '85.4/8.9/3.3/2.3) and the per-token block names the cause: between light and dark the ' +
  'same page moves sand 3.70 -> 28.62 and bone 37.20 -> 4.23, because tokens.css gives the ' +
  'two spec-sheet cards a Sand ground in dark. Counting that Sand as field would put this ' +
  "tool's dark phone near 80/4, which brackets round 4's 85.4/8.9 — so rounds 1-4 were not " +
  'putting Sand in Bone. This design does, on §2\'s own words ("sand: aged paper, spec ' +
  'sheets") and §1 trait 07 (bone, not white): Sand is the paper family, which is the ' +
  "counter's 30% budget, not the field's 60%. CONSEQUENCE: round 4's dark-phone figure " +
  'overstated Flare+Espresso by roughly the 28pp of Sand on that screen. The dark phone is ' +
  'not the worst screen in the matrix; on this mapping it is one of the best.'
...
export const SCROLLING_FORM_NOTE =
  ...
  'but a sequence of them. Measured on the tree of commit a2d4e6d (round 5, 2026-08-04), the ' +
  "app's 375 light document read 56.0/36.9/4.5/2.6 — deviation 15.0, apparently the best app " +
  'screen in the matrix — while its eight viewport windows read 41.6, 15.6, 44.9, 18.4, 65.7, ' +
  '91.1, 86.6 and 83.9 percent field: a Bone form stack at the head and a dark index sheet at ' +
  'the tail, averaging to a vector that appears on no screen, with a mean-of-screens deviation ' +
  'of 52.8. `deviation` above is the document reading and stays comparable with rounds 1-4; ' +
  '`scrollingForm` per row is this one. WHEN THEY DISAGREE, THE WINDOWS ARE THE TRUTH — they ' +
  'are what a reader is looking at. The document average moves when a card gets taller; only ' +
  'the windows move when a page gets better arranged, which is why the remaining work after ' +
  'round 5 is INTERLEAVING rather than more or less Flare.'
